    ...
    <div class="page-header">
        ...
        <nav class="breadcrumb-one" aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active" aria-current="page"><span>@lang('admin.product_categorys')</span></li>
            </ol>
        </nav>
            ...
            <div class="widget-content widget-content-area br-6">
                <div class="table-form">
                    <div class="form-group row">
                        ...
                        <a href="{{ route('dashboard.admin.product_categorys.create') }}" class="btn btn-primary">@lang('dashboard.add')</a>
                    </div>
                </div>
            </div>
        </div>
                ...
                <table class="display table" style="width:100%">
                    <thead>
                        <tr>
                            ...
                            <th>@lang('dashboard.size_image')</th>
                            <th>@lang('dashboard.created_at')</th>
                            <th>@lang('dashboard.action')</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach ($product_categorys as $index=>$categorey)

                            <tr>
                                <td>{{ $index+1 }}</td>
                                <td>{{ $categorey->name }}</td>
                                <td>{{ $categorey->size }}</td>
                                <td>{{ $categorey->created_at }}</td>
                                <td>
                                    <a href="{{ route('dashboard.admin.product_categorys.edit', $categorey->id) }}" data-toggle="tooltip" data-placement="top" title="Edit">
                                    ...
                                    <form method="post" action="{{ route('dashboard.admin.product_categorys.destroy', $categorey->id) }}" style="display: inline-block;">
                                        @csrf
                                        @method('delete')
